...
4. **「ターミナル」アプリを開き、以下を貼り付けて Enter**（初回のみ）

```
xattr -dr com.apple.quarantine /Applications/KeiHiPon.app
```

5. Launchpad または Applications フォルダから「KeiHiPon」を起動

> **なぜこの手順が必要ですか？**
> macOS はインターネットからダウンロードしたアプリに「隔離フラグ」を自動で付けます。Apple の有料審査（Developer Program）を受けていないアプリはこのフラグにより「壊れている」と表示されてブロックされます。上のコマンドはフラグを解除するもので、一度だけ実行すれば以降は普通に起動できます。
